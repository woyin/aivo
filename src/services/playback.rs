...
//! Two paths:
//!
//! 1. `play_interactive` — primary. With the `audio_playback` feature on,
//!    decodes the file in-process with [`rodio`], opens a sink on the
//!    system default output device, and on a TTY drives a small key loop
//!    (SPACE pause / ←→ seek / q quit) while streaming a status line.
//!    Falls through to (2) on rodio failure or when the feature is off.
//! 2. `play_external` — always available. Hands the file to a per-OS CLI
//!    player (`afplay` / `paplay` / PowerShell `SoundPlayer`). No pause /
//!    seek / position tracking — the function simply blocks until the
//!    child exits.
//!
//! Headless / minimal Linux servers can build with `--no-default-features`
//! (the published `aivo-linux-*` artifacts do, to avoid linking
//! `libasound.so.2`). Best-effort playback still works through the
//! external player path; `supports_streaming()` returns false so callers
//! switch to buffered generate-then-play instead of in-process streaming.
...
/// True when this build can do in-process streaming playback (rodio queue
/// fed chunk-by-chunk). Callers must gate streaming-eligibility on this:
/// without rodio there is no way to *play* the bytes as they arrive, so
/// the streaming path would just consume the API response and discard it.
pub fn supports_streaming() -> bool {
    cfg!(feature = "audio_playback")
}

/// Best-effort total-duration probe. With the `audio_playback` feature
/// on, opens `path` with rodio's decoder (no audio device touched) and
/// reads `Source::total_duration`. Returns `None` for formats whose
/// decoder can't report a length (some MP3s without Xing/VBRI headers,
/// or formats rodio can't decode at all) and for builds without rodio.
pub fn probe_duration(path: &Path) -> Option<Duration> {
    #[cfg(feature = "audio_playback")]
    {
        rodio_imp::probe_duration(path)
    }
...
/// Plays `path`. With the `audio_playback` feature on, drives rodio with
/// keyboard controls and falls through to the OS player if rodio can't
/// decode the file or no audio device is available. Without the feature,
/// hands the file straight to the OS player. `start_at` is the initial
/// seek offset (0 = beginning) and is ignored on the external path.
pub fn play_interactive(path: &Path, start_at: Duration) -> Result<PlaybackOutcome> {
    ...
    #[cfg(feature = "audio_playback")]
    {
        match rodio_imp::play_with_rodio(path, start_at) {
...
/// Streaming playback. Only meaningful when `supports_streaming()` is
/// true; without rodio there is no audio device to queue chunks onto.
/// On builds without `audio_playback` this drains `chunk_rx` (so the
/// producer thread doesn't deadlock on a full channel) and bails — but
/// callers must gate streaming on `supports_streaming()` first to avoid
/// burning an API request whose audio can't be played.
pub fn run_streaming_playback(chunk_rx: Receiver<Vec<u8>>) -> Result<PlaybackOutcome> {
    #[cfg(feature = "audio_playback")]
    {
        rodio_imp::run_streaming_playback(chunk_rx)
    }
    #[cfg(not(feature = "audio_playback"))]
    {
        while chunk_rx.recv().is_ok() {}
        bail!(
            "streaming playback is not built into this binary; \
             call supports_streaming() before taking the streaming path"
        )
    }
}
...
// ── Rodio-backed in-process playback (feature-gated) ─────────────────────
...
mod rodio_imp {
    ...
    pub(super) fn play_with_rodio(path: &Path, start_at: Duration) -> Result<PlaybackOutcome> {
        ...
        let sink =
            Sink::try_new(&handle).map_err(|e| anyhow::anyhow!("creating audio sink: {e}"))?;
        sink.append(decoder);
                    ...
                    Event::Key(KeyEvent {
                        ...
                        KeyCode::Right => {
                            ...
                            let target = match total {
                                Some(t) if target >= t => {
                                    t.saturating_sub(Duration::from_millis(100))
                                }
                                _ => target,
                            };
                            let _ = sink.try_seek(target);
    ...
    pub(super) fn run_streaming_playback(chunk_rx: Receiver<Vec<u8>>) -> Result<PlaybackOutcome> {
        let (_stream, handle) = OutputStream::try_default()
            .map_err(|e| anyhow::anyhow!("opening default audio output device: {e}"))?;
        let sink =
            Sink::try_new(&handle).map_err(|e| anyhow::anyhow!("creating audio sink: {e}"))?;
    ...
    fn print_streaming_seek_unavailable() {
        ...
        let _ = writeln!(
            ...
            "\r{}",
            crate::style::dim(
                "seek unavailable while streaming — wait for cache then seek on replay"
            )
        );
    }
...
mod tests {
    ...
    #[test]
    fn supports_streaming_matches_feature() {
        assert_eq!(supports_streaming(), cfg!(feature = "audio_playback"));
    }

    #[test]
    fn probe_duration_returns_none_for_missing_file() {
        // Always-true regardless of feature: missing file → None.
        assert!(probe_duration(Path::new("/definitely/does/not/exist.wav")).is_none());
    }
